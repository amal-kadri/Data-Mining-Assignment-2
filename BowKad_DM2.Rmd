...
# not_model = for Xtestif colnames(Xtest) not in colnames(Xtrain)

Xtrain = model.matrix(price ~ lotSize + livingArea  + bedrooms + bathrooms + age + sewer + waterfront + newConstruction - 1, data=saratoga_train)

Xtest = model.matrix(price ~ lotSize + livingArea  + bedrooms + bathrooms + age + sewer + waterfront + newConstruction, data=saratoga_test)
...
childY <- hotel_dev_train$children
childX <- sparse.model.matrix(children ~ .-arrival_date -1, data = hotel_dev_train)
childMatrix= t(sparse.model.matrix(children ~ .-arrival_date-1, data = hotel_dev_test))
...
predict(childBest, s = best_lambda, newx = hotel_dev_test)
################################
