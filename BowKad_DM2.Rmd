...
Xtest = model.matrix(price ~ lotSize + livingArea  + bedrooms + bathrooms + age + sewer + waterfront + newConstruction - 1, data=saratoga_test)
...
cv_House = foreach(k = k_grid, .combine = 'rbind')%do%{
  houseKModel = map(houseKNN$train, ~ knnreg(price ~ lotSize + livingArea 
                                           + bedrooms + bathrooms + age + sewerseptic + sewerpublic/commercial + sewernone + waterfront + newConstruction, k=k,
                                           data = ., use.all = FALSE))
