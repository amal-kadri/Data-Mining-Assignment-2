...
roc_curve_child = foreach(thresh = thresh_grid, .combine='rbind') %do% {
  hotel_test = ifelse(hotel_val_test >= thresh, 1, 0)
  confusion_out_logit = table(y = hotel_val_test$children, yhat_Lasso = hotel_val_test$childPred3)
  out_logit = data.frame(model = "logit",
                       TPR = confusedBestChild[2,2]/sum(hotel_val_test$children==1),
                       FPR = confusedBestChild[1,2]/sum(hotel_val_test$children==0))
...
ggplot(roc_curve_child) + 
  ...
  labs(title="ROC curve logit models") +
