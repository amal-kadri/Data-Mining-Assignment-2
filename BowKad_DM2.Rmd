...
DumbDev = DumbDev %>%
  select(-c(assigned_room_type, reserved_room_type))
...
#for all variables in mainVariables, if mainVariables[variable] not in colnames(MATRIX), add column of 0s for that variable name
...
f_include = function(mainVariables, variable_list, varaible_matrix) 
  for (i in mainVariables) {
    if (mainVariables[i] %notin% variable_list) {
      mutate(data = variable_matrix, mainVariables[i] <- 0)}
    }
...
#f_include(otherVariables, colnames(valTestMatrix), valTestMatrix)
...
variables = colnames(valTestMatrix)
...
#childLasso[["glmnet.fit"]][["beta"]]@Dimnames[[1]]
```
...
confusedBestChild
###################################################### label confusion matrices better
